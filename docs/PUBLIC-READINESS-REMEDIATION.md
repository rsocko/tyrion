...
| #99 | Dependency automation | Repository owner | **Configured.** Dependency graph, alerts, security updates, and weekly update groups are enabled. Current vulnerability remediation remains a publication gate below. | [`.github/dependabot.yml`](../.github/dependabot.yml), authenticated security APIs |
...
| #104 / #102 follow-up | Current Actions logs | Repository maintainer | **Resolved.** Every downloadable current log was scanned. Four runs had no downloadable log. Findings were limited to generic hosted-runner, CI, or generalized service-role paths. | Final issue #102 GitHub-surface scan |
