...
immutable commit enforcement. The protected default branch requires the baseline and
full validation jobs, an approving review, approval after the latest push, resolved
conversations, linear signed history, and current-branch checks. Administrators cannot
bypass the protection, and force pushes and deletion are disabled. Existing Actions
caches were removed; the workflows do not create replacements. Dependency alerts and
